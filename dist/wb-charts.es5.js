...
import { select, extent, merge, axisBottom, scaleUtc, axisLeft, axisRight, tickIncrement, range, transition, easeLinear, event, scaleLinear, utcSecond, utcMinute, utcHour, utcDay, utcMonth, utcWeek, utcYear, arc, bisector, scaleSequential, interpolateWarm, area, mean as mean$3, symbol, symbols, interpolate, line, lineRadial, interpolateArray, min, max, scaleBand, selectAll, mouse, interval, drag, format, scaleQuantize, scaleThreshold } from 'd3';
...
var CartesianAxis = /** @class */ (function (_super) {
    ...
    CartesianAxis.prototype.redraw = function (options) {
        ...
        if (this.options.x && this.options.x.domain) {
            ...
            for (var _i = 0, _a = this.charts; _i < _a.length; _i++) {
                ...
                xExtent = extent(merge([xExtent, [].concat.apply([], chartXExtent)]));
            }
        ...
        if (this.options.y && this.options.y.domain) {
            ...
            for (var _b = 0, _c = this.charts; _b < _c.length; _b++) {
                ...
                yExtent = extent(merge([yExtent, [].concat.apply([], chartYExtent)]));
            }
    ...
    CartesianAxis.prototype.updateGrid = function () {
        ...
        this.setCanvas();
        var that = this;
        var xAxis = axisBottom(this.xScale).ticks(5);
        var xGrid = axisBottom(this.xScale)
            .ticks(5)
        ...
        if (this.options.x && this.options.x.time) {
            ...
            var offsetScale = scaleUtc().domain(offsetDomain);
        ...
        var yAxis = axisLeft(this.yScale).ticks(5);
        var yGrid = axisRight(this.yScale)
            .ticks(5)
            .tickSize(this.width);
        if (this.options.y && this.options.y.axisType === 'degrees') {
            var domain = this.yScale.domain();
            var step = tickIncrement(domain[0], domain[1], 5);
            step = step >= 100 ? 90 : step >= 50 ? 45 : step >= 20 ? 15 : step;
            var start = Math.ceil(domain[0] / step) * step;
            var stop = Math.floor(domain[1] / step + 1) * step;
            yAxis.tickValues(range(start, stop, step));
            yGrid.tickValues(range(start, stop, step));
        }
    ...
    CartesianAxis.prototype.setRange = function () {
        if (!this.xScale) {
            this.xScale = this.options.x && this.options.x.time ? scaleUtc() : scaleLinear();
        }
        if (!this.yScale)
    ...
    CartesianAxis.prototype.generateMultiFormat = function () {
        ...
        return function (date) {
            ...
            return (utcSecond(offsetDate) < offsetDate
                ? m.format('.SSS')
...
var PolarAxis = /** @class */ (function (_super) {
    ...
    PolarAxis.prototype.redraw = function () {
        ...
        for (var _i = 0, _a = this.charts; _i < _a.length; _i++) {
            ...
            radialExtent = extent(merge([radialExtent, [].concat.apply([], chartRadialExtent)]));
        }
    ...
    PolarAxis.prototype.updateGrid = function () {
        ...
        var rAxis = axisBottom(this.radialScale).ticks(5);
        ...
        var angularTicks = range(this.angularRange[0], this.angularRange[1], this.angularRange[1] / 8);
    ...
    PolarAxis.prototype.setRange = function () {
        this.radialScale = scaleLinear().range([this.innerRadius, this.outerRadius]);
        this.angularScale = scaleLinear()
            .domain([0, 360])
...
var Chart = /** @class */ (function () {
    function Chart(data, options) {
        ...
        this.colorMap = scaleSequential(interpolateWarm);
    }
        ...
        get: function () {
            if (!this._extent) {
                ...
                var _loop_1 = function (key) {
                    ...
                    this_1._extent[path] = extent(this_1._data, function (d) {
                        return d[path];
                    });
                };
                var this_1 = this;
    ...
    Chart.prototype.mapDataCartesian = function (axis, dataKeys, domain) {
        ...
        var bisectData = bisector(function (d) {
            return d[xkey];
        });
...
var ChartArea = /** @class */ (function (_super) {
    ...
    ChartArea.prototype.plotterCartesian = function (axis, dataKeys) {
        ...
        if (this.options.colorScale === AUTO_SCALE) {
            colorScale.domain(extent(this.data, function (d) {
                return d[colorkey];
            }));
        }
        ...
        var area$1 = this.group.select('path');
        area$1.attr('d', areaGenerator(mappedData));
        area$1.datum(mappedData);
    };
...
var ChartMarker = /** @class */ (function (_super) {
    ...
    ChartMarker.prototype.plotterCartesian = function (axis, dataKeys) {
        ...
        elements
            ...
            .attr('d', symbol().type(symbols[symbolId]))
            .merge(elements)
    ...
    ChartMarker.prototype.plotterPolar = function (axis, dataKeys) {
        ...
        function arcTranslation(p) {
            ...
            return function (d, i, a) {
                var old = p[i];
                if (mean$1(old[tkey]) - mean$1(d[tkey]) > 180) {
                    old[tkey] = old[tkey] - 360;
                }
                else if (mean$1(old[tkey]) - mean$1(d[tkey]) < -180) {
                    old[tkey] = old[tkey] + 360;
                }
                var tInterpolate = interpolate(old[tkey], d[tkey]);
                var rInterpolate = interpolate(old[rkey], d[rkey]);
                return function (t) {
    ...
    Object.defineProperty(ChartRange.prototype, "extent", {
        get: function () {
            if (!this._extent) {
                ...
                var _loop_1 = function (key) {
                    var path = this_1.dataKeys[key];
                    var min$1 = min(this_1._data, function (d) {
                        if (d[path] === null)
                            return undefined;
                        return min(d[path]);
                    });
                    var max$1 = max(this_1._data, function (d) {
                        if (d[path] === null)
                            return undefined;
                        return max(d[path]);
                    });
                    this_1._extent[path] = [min$1, max$1];
                };
                var this_1 = this;
                for (var key in this.dataKeys) {
                    _loop_1(key);
    ...
    ChartRange.prototype.plotterCartesian = function (axis, dataKeys) {
        ...
        var mappedData = this.data.map(function (d) {
            return {
                ...
                y: d[ykey].map(axis.yScale),
                color: colorMap(colorScale(mean$2(d[colorkey])))
            };
        });
    ...
    ChartRange.prototype.plotterPolar = function (axis, dataKeys) {
        ...
        function arcTween(transition, p) {
            transition.attrTween('d', function (d, i, a) {
                var old = p[i];
                if (mean$2(old.t) - mean$2(d.t) > 180) {
                    old.t = old.t.map(function (x) {
                        return x - 360;
                    });
                }
                else if (mean$2(old.t) - mean$2(d.t) < -180) {
                    old.t = old.t.map(function (x) {
                        return x + 360;
                    });
                }
                var tInterpolate = interpolateArray(old.t, d.t);
                var rInterpolate = interpolateArray(old.r, d.r);
                return function (t) {
                    d.t = tInterpolate(t);
                    d.r = rInterpolate(t);
...
var ChartHistogram = /** @class */ (function (_super) {
    ...
    ChartHistogram.prototype.plotterCartesian = function (axis, dataKeys) {
        ...
        var histScale = scaleBand().domain(data.map(function (d) {
            return d[xkey];
        }));
...
var Legend = /** @class */ (function () {
    ...
    Legend.prototype.redraw = function () {
        ...
        var enter = entries
            ...
            .merge(entries)
            .each(function (d, i) {
            var entry = select(this);
            var chartElement = select(d.selector)
                .select('path')
                .node();
            if (chartElement) {
                ...
                entry.on('click', function () {
                    ...
                    if (display === 'visible') {
                        selectAll(d.selector).style('visibility', 'hidden');
                        entry.style('opacity', 0.5);
                    }
                    else {
                        selectAll(d.selector).style('visibility', 'visible');
...
var WarningLevels = /** @class */ (function () {
    ...
    WarningLevels.prototype.create = function (axis) {
        var scale = (this.scale = scaleLinear());
        ...
        this.warningAxis = axisRight(this.scale)
    ...
    WarningLevels.prototype.redraw = function () {
        ...
        var transition$1 = transition().duration(this.transitionTime);
        this.group
            .select('.y2-axis')
...
function dateFormatter(date, format, options) {
    var moment = momentTimezone$1(date).tz(options.timeZone);
    moment.locale('nl-NL');
    ...
    MouseOver.prototype.create = function (axis) {
        ...
        mouseG
            ...
            that.group
                ...
                var element = select(d).select('path');
            ...
            axis.canvas.selectAll('.mouse-per-line').attr('transform', function (d, i) {
                ...
                var mouseValue = axis.xScale.invert(mouse$1[0]);
            ...
            div.style('left', event.pageX + 'px').style('top', event.pageY - h + 'px');
        });
    };
...
var ZoomHandler = /** @class */ (function () {
    ...
    ZoomHandler.prototype.createHandler = function (axis) {
        ...
        if (this.mouseGroup.size() === 0) {
            ...
            mouseRect_1
                ...
                that_1.initSelection(mouse(this));
                document.addEventListener('mouseup', documentMouseUp_1);
            })
                .on('mouseup', function () {
                document.removeEventListener('mouseup', documentMouseUp_1);
                that_1.endSelection(mouse(this));
            })
                .on('dblclick', function () {
                that_1.resetZoom(mouse(this));
    ...
    ZoomHandler.prototype.initSelection = function (point) {
        ...
        this.mouseGroup.select('.overlay').on('mousemove', function () {
            that.updateSelection(mouse(this));
        });
        this.brushGroup
            .select('.select-rect')
    ...
    ZoomHandler.prototype.endSelection = function (point) {
        ...
        switch (this.mode) {
            case SelectionMode.X: {
                var xExtent = extent([point[0], this.brushStartPoint[0]].map(xScale.invert));
                this.axis.xScale.domain(xExtent);
                break;
            }
            case SelectionMode.Y: {
                var yExtent = extent([point[1], this.brushStartPoint[1]].map(yScale.invert));
...
var CurrentTime = /** @class */ (function () {
    ...
    CurrentTime.prototype.visit = function (axis) {
        ...
        var that = this;
        this.redraw();
        this.timer = interval(function (elapsed) {
            that.redraw();
...
var DstIndicator = /** @class */ (function () {
    ...
    DstIndicator.prototype.redraw = function () {
        var domain = this.axis.xScale.domain();
        var startMoment = momentTimezone$1(domain[0]).tz(this.axis.timeZone);
        var endMoment = momentTimezone$1(domain[1]).tz(this.axis.timeZone);
    ...
    DstIndicator.prototype.findDst = function (startMoment, endMoment) {
        ...
        var duration = momentTimezone$1.duration(m2.diff(m1)).asMinutes();
        while (duration > 1) {
            ...
            duration = momentTimezone$1.duration(m2.diff(m1)).asMinutes();
        }
...
var DataField = /** @class */ (function () {
    ...
    DataField.prototype.valueFormatter = function (d) {
        ...
        if (units.factor !== undefined) {
            var format$1 = units.precision !== undefined ? format(units.precision) : format(".1f");
            var valueString = value !== null ? format$1(value * units.factor) : '-';
...
var scaleWindCategories = scaleQuantize()
    .domain([-11.25, 371.25])
    .range([
    "N", "NNW", 'NW', "WNW",
    'W', "WZW", 'ZW', "ZZW",
...
var scaleBeaufort = scaleThreshold()
    .domain(beaufortLimits)
    .range(beaufortLabels);

export { ANTICLOCKWISE, AUTO_SCALE, Axis, CLOCKWISE, CartesianAxis, Chart, ChartArea, ChartHistogram, ChartLine, ChartMarker, ChartRange, CurrentTime, DataField, DstIndicator, Legend, LevelSelect, MouseOver, PolarAxis, WarningLevels, ZoomHandler, dateFormatter, mergeRecursive, scaleBeaufort, scaleWindCategories };
//# sourceMappingURL=wb-charts.es5.js.map
