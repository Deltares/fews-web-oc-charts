...
export class DstIndicator implements Visitor {
  ...
  redraw() {
    if ("x" in this.options) {
      ...
      let startDate = DateTime.fromJSDate(domain[0]).setZone(this.axis[axisIndex].timeZone)
      let endDate = DateTime.fromJSDate(domain[1]).setZone(this.axis[axisIndex].timeZone)
