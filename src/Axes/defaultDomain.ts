export function isDefaultD3Domain([start, end]: [number, number] | [Date, Date]): boolean {
...
export function isDefaultD3LinearScaleDomain([start, end]: [number, number]): boolean {
  return start === 0 && end === 1
}

// Checks if the provided date range is the default D3 time scale domain
// See: https://d3js.org/d3-scale/time#scaleTime
export function isDefaultD3TimeScaleDomain([start, end]: [Date, Date]): boolean {
  return (
...
export function isDateDomain(domain: [number, number] | [Date, Date]): domain is [Date, Date] {
